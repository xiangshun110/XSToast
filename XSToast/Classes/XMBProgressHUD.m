...
#endif  // (TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
{
	...
	XMBProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
{
    NSAssert(view, @"View must not be nil.");
	XMBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
{
    NSAssert(view, @"View must not be nil.");
	NSArray *huds = [XMBProgressHUD allHUDsForView:view];
	for (XMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self) {
		...
		self.animationType = XMBProgressHUDAnimationFade;
		self.mode = XMBProgressHUDModeIndeterminate;
		self.labelText = nil;
...
{
#if (TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE)
	if (animated && animationType == XMBProgressHUDAnimationZoomIn) {
		self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
	} else if (animated && animationType == XMBProgressHUDAnimationZoomOut) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == XMBProgressHUDAnimationZoomIn || animationType == XMBProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated
{
	...
	if (animated && showStarted) {
		...
		// in the done method
		if (animationType == XMBProgressHUDAnimationZoomIn) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
		} else if (animationType == XMBProgressHUDAnimationZoomOut) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
		}
...
- (void)updateIndicators
{
	...
	if (mode == XMBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		[self addSubview:indicator];
	} else if (mode == XMBProgressHUDModeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
	}
}
